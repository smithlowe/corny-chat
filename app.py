...
db = SQLAlchemy(app)
# Max buffer size set to 10MB for HD audio
...
def handle_connect():
    ...
    for msg in messages:
        emit('message', {'username': msg.username, 'message': msg.content, 'time': msg.timestamp, 'is_audio': msg.is_audio})
...
def handle_message(data):
    if data.get('message') == "/clear":
        ...
        return
    new_msg = Message(username=data['username'], content=data['message'], is_audio=data.get('is_audio', False))
